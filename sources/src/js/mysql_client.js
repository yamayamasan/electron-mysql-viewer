...
class MysqlClient {
    ...
    getConnectionDirect(config) {
        ...
        state.set('con_uuid', this.active, true);
        this.connections[this.active] = mysql.createConnection(config.mysql);
    ...
    static setConfig({ type, mysql, ssh }) {
        const config = {
            tunnel: {},
            mysql: {},
        };
        Object.assign(config.mysql, mysql);
        if (type === 2) {
            ...
            if (ssh.privateKey) {
                config.tunnel.privateKey = require('fs').readFileSync(ssh.privateKey);
            }
            if (ssh.disthost) {
                config.tunnel.dstHost = ssh.disthost;
            }
        }
